...
def flip_x(match):
    ...
    flipped_x = 500 - x
    return f"{flipped_x:g}"

def mirror_path(d_string):
    # Regex to find X coordinates. 
    # M x y, L x y
    # C x1 y1 x2 y2 x x y
    # We must properly parse SVG path commands.
    ...
    for cmd, argsStr in commands:
        ...
        # Using a regex that captures all numbers (including negatives and decimals)
        ...
        # Format the numbers back to string
        # Group by command required pairs
        if cmd == 'M' or cmd == 'L':
            parts = [f"{flipped_nums[i]:g} {flipped_nums[i+1]:g}" for i in range(0, len(flipped_nums), 2)]
            flipped_commands.append(f"{cmd}{', '.join(parts)}")
        elif cmd == 'C':
            parts = [f"{flipped_nums[i]:g} {flipped_nums[i+1]:g}" for i in range(0, len(flipped_nums), 2)]
            # C needs 3 pairs per segment
            # e.g., C x1 y1, x2 y2, x3 y3
            # Grouping into triplets of pairs
            cur_parts = []
...
left_muscles = [
    ...
    },
    {
        ...
        "d": "M250 350 L220 540 L250 610 Z"
    }
]

# Note: The fascia is centered. We will create l_fascia and r_fascia and combine them, 
# or just keep it as one path if mirrored nicely.
# Actually, the fascia is a diamond shape, so M250 350 L220 540 L250 610 L280 540 Z
...
for m in all_muscles:
    react_code += f'  {{ id: "{m["id"]}", name: "{m["name"]}",\n    d: "{m["d"]}" }},\n'
...
with open("c:/Users/zetz1/OneDrive/Desktop/Progress Note with Grok/pt-progress-note/components/DetailedPosteriorMuscleDiagram.tsx", "w", encoding="utf-8") as f:
    f.write(react_code)

print("Done generating React component!")
